...
for elefini in elemento:
    ...
    # numero minimo de nodo seleccionado
    min_nodenum_cold = int(
        mapdl.get("min_nodenum_cold", "node", "0", "num", "min"))
    # numero maximo de nodo seleccionado
    max_nodenum_cold = int(
        mapdl.get("max_nodenum_cold", "node", "0", "num", "max"))
    ...
    # numero minimo de nodo seleccionado
    min_nodenum_hot = int(
        mapdl.get("min_nodenum_hot", "node", "0", "num", "min"))
    # numero maximo de nodo seleccionado
    max_nodenum_hot = int(
        mapdl.get("max_nodenum_hot", "node", "0", "num", "max"))
    ...
    while iteracion < 3:
        print(iteracion)
        # Calculo Tm
        # lista con todas las areas interiores (numeros correctos, saltandose la que corresponde)
        areas_convection = []
        ...
        for idx, i in enumerate(areas_convection):
            mapdl.asel("S", vmin=i)
            mapdl.nsla()
            # temperaturas en cada nodo del area
        ...
        for i in range(0, N_CAVITIES):
            ...
            # selecciona areas
            mapdl.asel("S", vmin=a_conv[cuadrupleta[0]], vmax=a_conv[cuadrupleta[-1]])
