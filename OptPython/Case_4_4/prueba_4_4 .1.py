...
N_CAV_L = [4,3,4,3,4]
N_CAV_W = 5
n_cav = (n_cav_L-1)*n_cav_W//2+n_cav_L*(n_cav_W-n_cav_W//2)
CAV_PER_ROW = [4,3,4,3,4]
...
x = [46, 46, 46, 46, 46, 46, 46, 46,46, 46, 46, 46,46, 46, 46,46,46,46, 42, 118, 61]
...
suma = 0
for i in range(n_cav_L, n_cav_L*2-1):    
    suma = suma + x[i]
tlb = (L-suma)/(n_cav_L) #mm

suma = 0
for i in range(n_cav_L*2-1, n_cav_L*3-1):    
    suma = suma + x[i]
tlc = (L-suma)/(n_cav_L+1) #mm

suma = 0
for i in range(n_cav_L*3-1, n_cav_L*4-3):    
    suma = suma + x[i]
tld = (L-suma)/(n_cav_L+1) #mm

suma = 0
for i in range(n_cav_L*4-1, n_cav_L*4-5):    
    suma = suma + x[i]
tld = (L-suma)/(n_cav_L+1) #mm

tw = (W-n_cav_W*x[-3])/(n_cav_W+1) #mm

print(tla,tlb,tlc,tld)
...
cb = np.concatenate((c1b,c2b,c3b))
cc = np.concatenate((c1c,c2c,c3c,c4c))
cd = np.concatenate((c1d,c2d,c3d))

# Thermal parameters
lambda_clay = x[-1]/100 #W/mK

matriz = np.array([c1a,c2a,c3a,c4a,c1b,c2b,c3b,c1c,c2c,c3c,c4c,c1d,c2d,c3d])

#elemento = [0.0018,0.0017,0.0016,0.0015,0.0014,0.0013,0.0012,0.0011,0.00115,0.00112,0.00111]
elemento = [0.004]
